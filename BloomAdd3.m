...
% BloomAdd2 Adds an element to the Bloom Filter using Enhanced Double Hashing.

    % Get the size of the Bloom Filter
    BFSize = length(BF);

    % Generate two independent hash values
    h1 = string2hash(element, 'sdbm'); % Uniform primary hash
    h2 = string2hash(element, 'djb2'); % Uniform secondary hash
